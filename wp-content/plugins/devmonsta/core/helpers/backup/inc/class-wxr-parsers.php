...
class Devm_WXR_Parser_SimpleXML
{
	function parse($file)
	{
		...
		// css 
		$elementor_file = [];
		foreach ($xml->xpath('/rss/channel/wp:elementor') as $elementor_arr) {
			$a = $elementor_arr->children($namespaces['wp']);
			$temp_var = (array) $a->path;
			if (is_array($temp_var) && isset($temp_var[0])) {
				$elementor_file[] = $temp_var[0];
			}
		}

		// grab sidebar
		$sidebar_widget = "";
		foreach ($xml->xpath('/rss/channel/wp:sidebar') as $sidebars) {
			$sidebar = $sidebars->children($namespaces['wp']);
			$widget = (array) $sidebar->widgets;
			if (isset($widget[0])) {
		...
		foreach ($xml->xpath('/rss/channel/wp:customizer') as $customizers) {
			$customizer = $customizers->children($namespaces['wp']);
			$themes = (array) $customizer->theme;
			foreach ($themes as $theme) {
				array_push($theme_mod_array, $theme);
			}
		}
