<div class="shopengine-filter-single">
	<form
		...
		data-default-range="<?php echo wp_json_encode( $default_range ); ?>" data-exchange-rate="<?php echo apply_filters('shopengine_currency_exchange_rate', 0);?>">
		...
		*/ ?>
		...
		<div class="shopengine-filter-price-btns">
				<p class="shopengine-filter-price-result" data-sign="<?php echo get_woocommerce_currency_symbol(); ?>">
					<?php
						$format = apply_filters('shopengine_product_filter_currency_symbol_format', '%1$s%2$s'); 
						echo sprintf($format, get_woocommerce_currency_symbol(), esc_html($default_range[0]))  .' - '. sprintf($format, get_woocommerce_currency_symbol(), esc_html($default_range[1]));
					?>
				</p>
				<button type="button" class="shopengine-filter-price-reset"><?php echo esc_html__( 'Reset', 'bascart-essential' ); ?></button>
		</div>
	</form>
</div>