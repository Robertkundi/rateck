...
class Admin_Product {
	...
	public function get_option_terms_product($taxonomy, $index) {
        ...
        <select multiple="multiple" data-placeholder="<?php esc_attr_e('Select terms', 'bascart-essential'); ?>"
        ...
        <button class="button plus select_all_attributes"><?php esc_html_e('Select all', 'bascart-essential'); ?></button>
        <button class="button minus select_no_attributes"><?php esc_html_e('Select none', 'bascart-essential'); ?></button>
        <button class="button fr plus shopengine_assign_new_attribute" data-type="<?php echo esc_attr( $taxonomy->attribute_type ) ?>">
			<?php esc_html_e('Add new', 'bascart-essential'); ?>
        </button>
	...
	public function add_attribute_by_ajax() {
		...
		if(!wp_verify_nonce($nonce, 'shopengine_nonce_add_attribute')) {
			wp_send_json_error(esc_html__('Request denied', 'bascart-essential'));
		}

		if(empty($name) || empty($swatch) || empty($tax) || empty($type)) {
			wp_send_json_error(esc_html__('Insufficient data', 'bascart-essential'));
		}

		if(!taxonomy_exists($tax)) {
			wp_send_json_error(esc_html__('Taxonomy is not exists', 'bascart-essential'));
		}

		if(term_exists($name, $tax)) {
			wp_send_json_error(esc_html__('Term already exists', 'bascart-essential'));
		}
		...
		wp_send_json_success(
			[
				'msg'  => esc_html__('Successfully added', 'bascart-essential'),
				'id'   => $term->term_id,
				'slug' => $term->slug,
				'name' => $term->name,
			]
	...
	public function shopengine_term_template() {
        ...
        <div id="shopengine_tpl_modal" class="shopengine_modal_container">
            <div class="shopengine_modal">
                ...
                <div class="shopengine_modal__header"><h2><?php esc_html_e('Add new term', 'bascart-essential') ?></h2></div>
                <div class="shopengine_modal__content">
                    <p class="shopengine_term__name">
                        <label>
							<?php esc_html_e('Name', 'bascart-essential') ?>
                            <input type="text" class="shopengine__input" name="name">
                        </label>
                    </p>
                    <p class="shopengine_term__slug">
                        <label>
							<?php esc_html_e('Slug', 'bascart-essential') ?>
                ...
                <div class="shopengine_modal__footer">
                    <button class="button button-secondary shopengine_modal__close"><?php esc_html_e('Cancel', 'bascart-essential') ?></button>
                    <button class="button button-primary shopengine_add_attribute_submit"><?php esc_html_e('Add New', 'bascart-essential') ?></button>
        ...
        <script type="text/template" id="tmpl-shopengine__tpl_input__color">
            ...
            <label><?php esc_html_e('Color', 'bascart-essential') ?></label><br>
        ...
        <script type="text/template" id="tmpl-shopengine__tpl_input__image">
            ...
            <label><?php esc_html_e('Image', 'bascart-essential') ?></label><br>

            <div class="shopengine_term_img_thumbnail" style="float:left;margin-right:10px;">
                <img src="<?php echo esc_url(Helper::get_dummy()) ?>" width="60px" height="60px" alt="<?php esc_attr_e('swatch term thumbnail','bascart-essential'); ?>"/>
            </div>
            ...
            <div style="line-height:60px;">
                ...
                <button type="button" class="shopengine_upload_img_button button">
					<?php esc_html_e('Upload/Add image', 'bascart-essential'); ?>
                </button>

                <button type="button" class="shopengine_remove_img_btn button hidden">
					<?php esc_html_e('Remove image', 'bascart-essential'); ?>
                </button>
            </div>
        ...
        <script type="text/template" id="tmpl-shopengine__tpl_input__label">
            ...
            <label>
				<?php esc_html_e('Label', 'bascart-essential') ?>
